...
format_table <- function(new_bike_tbl) {
  ...
  new_bike_tbl %>%
    ...
    pivot_longer(cols = -model, names_to = "New Model Attribute",
...
plot_bike_prediction <- function(data, interactive = TRUE ) {
  ...
  g <- data %>%
    ...
    geom_violin() +
    geom_jitter(aes(text = label_text), size = 2, width = 0.1, alpha = 0.7) +
...
bind_bike_prediction(bikes_tbl, new_bike_tbl) %>%
  plot_bike_prediction(interactive = FALSE) # FALSE if you want static report / PDF doc


# 8.3 Save functions ----

scale_color_todd_dark <- function(...){
  library(scales)
...
dump(c("generate_new_bike", "format_table", "bind_bike_prediction",
       "plot_bike_prediction", "scale_color_todd_dark", "theme_dark_grey"),
