---
title: "Sales Dashboard - By Todd Warczak"
output:
...
runtime: shiny
resource_files:
- 00_data/bikes_database.db
- 00_scripts/03_make_predictions.R
- 00_scripts/04_demand_forecast.R
---
...
source(here::here("00_scripts", "04_demand_forecast.R"))
source(here::here("00_scripts", "03_make_predictions.R")) # for ggplot themes
...
```{r}
con <- dbConnect(RSQLite::SQLite(), here::here("00_data", "bikes_database.db"))
...
renderValueBox({
  ...
  valueBox(
    value   = summary_values_tbl()$wealth_metric %>% scales::dollar(scale = 1e-6,
                                                                    suffix = "M",
                                                                    accuracy = 0.1), 
    caption = "Sales", 
    icon    = "fa-money-bill-wave",
    #icon    = "fa-money-check-alt", 
    color   = case_when(summary_values_tbl()$wealth_metric < 5000000 ~ "danger",
                        summary_values_tbl()$wealth_metric < 10000000 ~ "warning",
                        TRUE ~ "success"))
...
output$plotly_2 <- renderPlotly({
  ...
  ifelse(input$forecast_mode == TRUE & input$view_mode == TRUE,
         p <- time_plot_predictions_tbl() %>% 
           plot_forecast(), 
         ifelse(input$forecast_mode == FALSE & input$view_mode == TRUE,
                p <- time_plot_tbl() %>% 
                  plot_time_series(),
                ifelse(input$forecast_mode == TRUE & input$view_mode == FALSE,
                       p <- time_plot_predictions_tbl() %>% 
                         plot_forecast_dark(),
                       p <- time_plot_tbl() %>% 
