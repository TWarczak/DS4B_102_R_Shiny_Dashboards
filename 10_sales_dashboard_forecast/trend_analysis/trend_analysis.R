...
source(here::here("10_sales_dashboard_forecast",
                  "trend_analysis",
                  "model_forecast_xgb.R"))

processed_data_tbl %>%
    aggregate_time_series("month") %>%
    generate_forecast_xgb(n_future = 12, seed = 123,
                          mtry = 50,
                          trees = 500,
                          min_n = 5,
                          tree_depth = 6,
...
source(here::here("10_sales_dashboard_forecast",
                  "trend_analysis",
                  "model_forecast_glmnet.R"))

processed_data_tbl %>%
    aggregate_time_series("weekly") %>%
    generate_forecast_glmnet(n_future = 52, seed = 123,
